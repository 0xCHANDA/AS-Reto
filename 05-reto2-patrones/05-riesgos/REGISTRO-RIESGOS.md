...
|---|---|---:|---:|---:|---|---|
| R-01 | Si se introduce una categoría de res y no queda cubierta correctamente por los creadores registrados, el alta puede fallar de forma explícita; si una categoría desconocida aparece al reconstruir datos persistidos, el fallback legacy puede recargarla silenciosamente como `Ternero`, produciendo pérdida de fidelidad del tipo. | 2 | 4 | 8 | Registrar el creador, probar fronteras, probar reconstrucción desde persistencia y verificar las categorías persistidas. | **Alta:** `Ninguna categoría de res cubre una edad de N meses.` **Persistencia:** categoría desconocida → fallback legacy a `Ternero`, sin excepción. |
| R-02 | Si un `IVacunaBuilder` nuevo no respeta el contrato de variante y mensajes, cambia el texto que ve el usuario al crear una vacuna o un lote. | 3 | 4 | 12 | Revisar los cinco miembros del contrato: `Variante`, `VariantePlural`, `VarianteLote`, `DetalleEspecifico` y `NombreEnResumenDeLote`. Después correr la comparación de salidas, que incluye el literal `{nombre}` sin interpolar. | Falla una de las 98 verificaciones, o aparece una diferencia en la comparación de salidas. |
