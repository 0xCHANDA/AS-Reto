...
| **Qué sale y qué entra** | Salen: las siete suscripciones acumulativas dentro de métodos. Entran: `IObservadorMensaje` (Observer), `RecolectorMensajes` (ConcreteObserver) y `CapturaMensajes`, que delimita qué operación lee qué avisos. `Hacienda` conserva sus conexiones permanentes durante la inicialización; los publishers de `Potrero` se conectan mediante `Potrero.ConectarObservador(...)`, cuyo guard evita registrar handlers duplicados. |
| **Cómo se relaciona** | Lo construye `Hacienda.cs:52` como campo. Lo usan los tres publishers de `Hacienda` y los cuatro de cada `Potrero`; cada operación abre su `Capturar()`. No interactúa con los otros dos patrones. `PublisherVacunaVencida` queda sin observador ni invocación. |
